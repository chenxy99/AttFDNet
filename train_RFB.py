...
num_classes = (81, 16)[args.dataset == 'VOC']
...
if args.resume_net == None:
    ...
    def zeros_init(m):
        for key in m.state_dict():
            if key.split('.')[-1] == 'weight':
                init.constant_(m.state_dict()[key], 0)
            elif key.split('.')[-1] == 'bias':
                m.state_dict()[key][...] = 0
    ...
    net.GCBlock.apply(weights_init)
    net.GCBlock.conv_WV2.apply(zeros_init)
...
def train():
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if args.ngpu > 1:
            net.module.weight_source_norm()
        else:
            net.weight_source_norm()
