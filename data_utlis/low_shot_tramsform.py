def VOC_low_shot_dataset_transform(dataset, num_shot, num_classes):
    ...
    np.save("voc2007_record.npy", record)
    '''

    '''
    record = np.load("voc2007_record.npy")

    for index in range(len(dataset.ids)):
        record_index = record[index]
        if (record_index > 0).sum() < 2:
            record[index] = 0
    ...
    used_index = [135, 201, 739, 1421, 1531, 1595, 1646, 2034, 9, 10, 34, 35, 95, 105, 118, 119, 154, 167, 180, 198,
                  199, 208, 209, 220, 225, 246, 260, 264, 276, 288, 292, 316, 346, 350, 368, 379, 398, 452, 457, 458,
                  461, 463, 469, 474, 488, 490, 511, 515, 534, 536, 572, 576, 593, 613, 615, 622, 666, 670, 675, 684,
                  694, 697, 701, 704, 709, 710, 714, 720, 721, 728, 731, 738, 740, 743, 747, 750, 754, 755, 800, 814,
                  819, 821, 836, 838, 845, 855, 856, 867, 876, 885, 886, 898, 902, 946, 1026, 1045, 1053, 1106, 1134,
                  1204]

    # IL_10_shot 11-20
    used_index = [8, 77, 113, 129, 279, 298, 333, 384, 386, 415, 456, 501, 511, 541, 581, 582, 607, 672, 742, 745, 765,
                  791, 804, 831, 843, 874, 882, 890, 908, 923, 924, 930, 1054, 1106, 1118, 1128, 1130, 1185, 1192, 1197,
                  1206, 1207, 1210, 1212, 1216, 1217, 1218, 1219, 1223, 1229, 1267, 1300, 1347, 1350, 1374, 1433, 1434,
                  1440, 1466, 1475, 1660, 2319, 2994, 3087, 31, 73, 76, 78, 126, 178, 184, 187, 195, 201, 202, 218, 232,
                  240, 249, 252, 259, 265, 278, 291, 302, 308, 316, 323, 354, 363, 373, 379, 385, 400, 407, 442, 469,
                  480, 503, 506]
    temp = []
    for index in used_index:
        temp.append(dataset.ids[index])
    ...
    maindir = '/media/data2/xychen/IL-30-shots-txt/target3/'
